...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: submissions, error: serr } = await supa
      .from('submissions')
      .select('id, problem_id, student_id, answer_image_url, created_at, users:student_id ( display_name ), grades:grades ( score_numeric, score_max, feedback_text )');
    if (serr) throw serr;
    ...
    for (const s of submissions || []) {
      ...
      const score_numeric = (s as any).grades?.[0]?.score_numeric ?? 0;
      const score_max = (s as any).grades?.[0]?.score_max ?? 1;
      ...
      row.problem_scores.push({ problem_id, score: score_numeric, max: score_max });
